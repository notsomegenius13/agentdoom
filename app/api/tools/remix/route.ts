...
export async function POST(req: NextRequest) {
  ...
  if (!filterResult.allowed) {
    return new Response(JSON.stringify({ error: filterResult.reason, category: filterResult.category }), {
      status: 403,
      headers: { 'Content-Type': 'application/json' },
    })
  }
    ...
    async start(controller) {
      ...
      try {
        ...
        if (result.url) {
          ...
          await sql`
            INSERT INTO tools (id, slug, title, description, prompt, category, preview_html, remixed_from, deploy_url, generation_time_ms, status)
