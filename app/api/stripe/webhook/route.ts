...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  if (userId && session.mode === 'subscription' && session.customer) {
    ...
    await db`
      ...
      WHERE id = ${userId}
    `
    console.log(`Pro subscription started: user=${userId} customer=${customerId}`)
  ...
  console.log(`Purchase recorded: tool=${toolId} buyer=${buyerId} amount=${session.amount_total}`)
...
async function handlePaymentSucceeded(paymentIntent: Stripe.PaymentIntent) {
  ...
  if (remixRoyaltyCents > 0 && originalCreatorAccount) {
    ...
    console.log(`Remix royalty of ${remixRoyaltyCents}c sent to ${originalCreatorAccount}`)
  }
}
...
async function handleAccountUpdated(account: Stripe.Account) {
  ...
  console.log(`Account updated: user=${userId} charges=${account.charges_enabled} payouts=${account.payouts_enabled}`)
}
...
async function handleSubscriptionChange(subscription: Stripe.Subscription) {
  ...
  console.log(`Subscription ${subscription.id}: status=${subscription.status} active=${isActive}`)
}
