...
export async function runPipeline(opts: PipelineOptions): Promise<PipelineResult> {
  ...
  if (cacheResult.hit && cacheResult.config && cacheResult.classification) {
    ...
    emit(
      'classify',
      `Cache hit (${Math.round(cacheHit!.similarity * 100)}% match: "${cacheHit!.matchedPrompt}")`,
    );
  }
  ...
  emit(
    'classify',
    `Category: ${classification.category} (complexity: ${classification.complexity}/10)`,
    {
      classification,
    },
  );
  ...
  emit('assemble', `Assembled ${html.length} bytes of HTML`, { html });
  ...
  if (validation.overallVerdict === 'fail') {
    emit(
      'validate',
      `Validation failed at ${validation.retryContext?.failedStage || 'unknown'} stage`,
    );
  } else {
  ...
  if (!scanResult.safe) {
    ...
    const blockedModeration: ModerationResult = {
      ...
      flags: scanResult.findings.map((f) => ({
        category:
          f.category === 'xss' || f.category === 'script_injection'
            ? ('malware' as const)
            : ('malware' as const),
  ...
  if (moderation.verdict === 'review') {
    const flags = moderation.flags
      ...
      .map((f) => f.category)
      .join(', ');
    emit('moderate', `Flagged for review: ${flags} (risk: ${moderation.riskScore}/100)`, {
      moderation,
    });
  } else {
  ...
  const deployResult = await deploy({
    ...
    slug,
    persist: {
      title: config.title || classification.suggestedTitle || 'Untitled Tool',
      description: config.description || `Generated ${classification.category} tool`,
      prompt,
      category: classification.category,
      config,
      modelUsed: 'forge-pipeline',
    },
  });
